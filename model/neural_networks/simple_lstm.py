...
def vectorize_sentences(sentences, values, word_idx, story_maxlen, query_maxlen):
    X = []
    # Xt = []
    ...
    return sequence.pad_sequences(X, maxlen=query_maxlen), np.array(values)
...
if __name__ == "__main__":
    ...
    sens, values = read_data('../data/output/news_gra_sen_title_train.csv')
    sens_test, values_test = read_data('../data/output/news_gra_sen_title_test.csv')
    ...
    vocab = sorted(set(flatten_sens + flatten_sens_test))
    # print vocab
    ...
    # doc_maxlen = max(map(len, (x for x in docs)))
    sen_maxlen = max(map(len, (x for x in sens)))
    maxlen = sen_maxlen


    x_train_b, y_train = vectorize_sentences(sens, values, word_idx, maxlen, maxlen)
    x_test_b, y_test = vectorize_sentences(sens_test, values_test, word_idx, maxlen, maxlen)
    ...
    model = Sequential()
    model.add(Embedding(vocab_size, EMBED_HIDDEN_SIZE, input_length=maxlen, dropout=0.1))
    model.add(LSTM(EMBED_HIDDEN_SIZE, dropout_W=0.1, dropout_U=0.1))
    ...
    print('Training')
    model.fit(x_train_b, y_train, batch_size=BATCH_SIZE, nb_epoch=5, validation_split=0.05)

    predicts = prepare_predicts(x_test_b)
