...
def tokenize(sent):
    ...
    # 如果单词中出现数字,则统一归为一类,用*表示. 因为我想带有数字的标记可能会有更大的可能性更为重要句子
...
if __name__ == "__main__":
    ...
    # 读取数据, 把train集,test集中 sentence, title 和tag信息分别放到数组中
    print("Reading CSV file...")
    docs, sens, titles, values = read_data('data/28/news_doc_sen_title_wiki_sample_train.csv')
    print(docs[0])
    print(sens[0])
    print(titles[0])
    print(values[0])
    docs_test, sens_test, titles_test, values_test = read_data('data/28/news_doc_sen_title_wiki_sample_test.csv')
    ...
    # 将sentence和title的二维数组扁平化,组成一个一位数组
    ...
    # 创建单词字典,把train集和test集中的所有出现的单词放入字典中
